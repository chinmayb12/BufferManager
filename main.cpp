...
void simulate::processSelectQuery(FILE *file_pointer, int col1, string value,int comp){
    int numPages = 0;
    ...
    for(int i=0;i<numPages;++i){
        ...
        while(numRecords--){
            ...
            memcpy(&salary, data+20*sizeof(char), sizeof(int));
            memcpy(&emp_no, data+20*sizeof(char)+sizeof(int), sizeof(int));
            data += recordSize;
            numLeft -= recordSize;
            if(col1==2){
                if(comp==1 && salary == atoi(value.c_str())){
                    cout<<name<<" "<<salary<<" "<<emp_no<<endl;
                }
                else if(comp==2 && salary > atoi(value.c_str())){
                    cout<<name<<" "<<salary<<" "<<emp_no<<endl;
                }
                else if(comp==3 && salary < atoi(value.c_str())){
                    cout<<name<<" "<<salary<<" "<<emp_no<<endl;
                }
            }
            else if(col1==3 && emp_no == atoi(value.c_str())){
                if(comp==1){
                    cout<<name<<" "<<salary<<" "<<emp_no<<endl;
                }
                else{
                    cout<<"No comparison possible\n";
                }
            }
...
int main(){
    ...
    if(queryType == 1){
        int subquerytype;
        cout<<"Enter 1 for equality, 2 for greater than, 3 for less than: ";
        cin>>subquerytype;
        cout<<"Enter column number of Database: ";
        int col;
        cin>>col;
        if(subquerytype == 1){
            cout<<"Enter the value to be equal to: ";
        }
        else if(subquerytype == 2){
            cout<<"Enter the value to be greater than: ";
        }
        else if(subquerytype == 3){
            cout<<"Enter the value to be less than: ";
        }
        
        string value;
        cin>>value;
        s.processSelectQuery(file_pointer1, col, value, subquerytype);
    }
    else if(queryType == 2){
